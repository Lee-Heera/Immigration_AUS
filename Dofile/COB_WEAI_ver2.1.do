...
************************ pilot study*************************
***********************outcome variable - version 2.1. 
****** section1 
est clear 
xi: xtivreg2 growunempl_rate (Xit2 = Zit2) i.year $demo if sample==1, fe cluster(LGAFINAL21) robust first savefprefix(fs_)  // 감소 
est store m1

xi: xtivreg2 growmanu_share (Xit2 = Zit2) i.year $demo if sample==1, fe cluster(LGAFINAL21) robust first savefprefix(fs_) // 증가 
est store m2 
...
***** section 2 - 서비스업 세부 산업 
est clear 
xi: xtivreg2 growconsum_serv_share (Xit2 = Zit2) i.year $demo if sample==1, fe cluster(LGAFINAL21) robust first savefprefix(fs_) 
est store m1

xi: xtivreg2 growproduc_serv_share (Xit2 = Zit2) i.year $demo if sample==1, fe cluster(LGAFINAL21) robust first savefprefix(fs_)  // 감소 
est store m2  

xi: xtivreg2 growpublic_serv_share (Xit2 = Zit2) i.year $demo if sample==1, fe cluster(LGAFINAL21) robust first savefprefix(fs_) 
est store m3 

esttab m* , nogap stats(N widstat cdf arf arfp) title("Table 1C: FEIV")  r2(%8.3f) b(%8.3f) se(%8.3f) label star(* 0.10 ** 0.05 *** 0.01)
	
***** section 3 - jobs by skill level 
est clear 
xi: xtivreg2 growhighsk_share (Xit2 = Zit2) i.year $demo if sample==1, fe cluster(LGAFINAL21) robust first savefprefix(fs_) 
est store m1

xi: xtivreg2 growmidsk_share (Xit2 = Zit2) i.year $demo if sample==1, fe cluster(LGAFINAL21) robust first savefprefix(fs_) // 증가 
est store m2 

xi: xtivreg2 growlowsk_share (Xit2 = Zit2) i.year $demo if sample==1, fe cluster(LGAFINAL21) robust first savefprefix(fs_) // 감소 
est store m3 

esttab m* , nogap stats(N widstat cdf arf arfp) title("Table 1C: FEIV")  r2(%8.3f) b(%8.3f) se(%8.3f) label star(* 0.10 ** 0.05 *** 0.01)

***********************outcome variable - version 2.2. 
est clear 
xi: xtivreg2 grow2unemployed (Xit2 = Zit2) i.year $demo if sample==1, fe cluster(LGAFINAL21) robust first savefprefix(fs_) 
est store m1

xi: xtivreg2 grow2manu_tot (Xit2 = Zit2) i.year $demo if sample==1, fe cluster(LGAFINAL21) robust first savefprefix(fs_) // 증가 
est store m2 

xi: xtivreg2 grow2service_tot (Xit2 = Zit2) i.year $demo if sample==1, fe cluster(LGAFINAL21) robust first savefprefix(fs_)  // 증가 
est store m3 
...
xi: xtivreg2 grow2public_serv (Xit2 = Zit2) i.year $demo if sample==1, fe cluster(LGAFINAL21) robust first savefprefix(fs_) // 증가 
est store m3 

esttab m* , nogap stats(N widstat cdf arf arfp) title("Table 1C: FEIV")  r2(%8.3f) b(%8.3f) se(%8.3f) label star(* 0.10 ** 0.05 *** 0.01) 
***** section 3 - jobs by skill level 
est clear 
xi: xtivreg2 grow2highsk_tot (Xit2 = Zit2) i.year $demo if sample==1, fe cluster(LGAFINAL21) robust first savefprefix(fs_) // 증가 
est store m1

xi: xtivreg2 grow2midsk_tot (Xit2 = Zit2) i.year $demo if sample==1, fe cluster(LGAFINAL21) robust first savefprefix(fs_) //증가 
est store m2 
...
***********************outcome variable - version 2.3. 
est clear 
xi: xtivreg2 dlog_unemployed (Xit2 = Zit2) i.year $demo if sample==1, fe cluster(LGAFINAL21) robust first savefprefix(fs_) 
est store m1

xi: xtivreg2 dlog_manu_tot (Xit2 = Zit2) i.year $demo if sample==1, fe cluster(LGAFINAL21) robust first savefprefix(fs_) // 증가 
est store m2 

xi: xtivreg2 dlog_service_tot (Xit2 = Zit2) i.year $demo if sample==1, fe cluster(LGAFINAL21) robust first savefprefix(fs_)  // 증가 
est store m3 

esttab m* , nogap stats(N widstat cdf arf arfp) title("Table 1C: FEIV")  r2(%8.3f) b(%8.3f) se(%8.3f) label star(* 0.10 ** 0.05 *** 0.01)
***** section 2 - 서비스업 세부 산업 
est clear 
xi: xtivreg2 dlog_consum_serv (Xit2 = Zit2) i.year $demo if sample==1, fe cluster(LGAFINAL21) robust first savefprefix(fs_) // 증가 
est store m1

xi: xtivreg2 dlog_produc_serv (Xit2 = Zit2) i.year $demo if sample==1, fe cluster(LGAFINAL21) robust first savefprefix(fs_) 
est store m2 

xi: xtivreg2 dlog_public_serv (Xit2 = Zit2) i.year $demo if sample==1, fe cluster(LGAFINAL21) robust first savefprefix(fs_) // 증가 
est store m3 
...
***** section 3 - jobs by skill level 
est clear 
xi: xtivreg2 dlog_highsk_tot (Xit2 = Zit2) i.year $demo if sample==1, fe cluster(LGAFINAL21) robust first savefprefix(fs_) // 증가 
est store m1

xi: xtivreg2 dlog_midsk_tot (Xit2 = Zit2) i.year $demo if sample==1, fe cluster(LGAFINAL21) robust first savefprefix(fs_) //증가 
est store m2. 
...
************************* 1. 전체 main table ******************
*******************************Version 2.1.*************************
cd "/Users/ihuila/Desktop/data/2025ABS/tables/2-1"
*********************** Table 1 -> 여기부터 코드 수정 
// Table1 (Panel A)
est clear 

xi: reg growunempl_rate Xit2 i.year  if sample==1, vce(cluster LGAFINAL21)
est store reg1 
...
// Table 1 (Panel B) : FE
est clear
xi: xtivreg2 growunempl_rate Xit2 i.year if sample==1, fe cluster(LGAFINAL21) savefprefix(fs_) 
est store m1
...
est clear 
xi: xtivreg2 growunempl_rate (Xit2 = Zit2) i.year $demo if sample==1, fe cluster(LGAFINAL21) robust first savefprefix(fs_) 
est store m1
...
xi: reg growconsum_serv_share Xit2 i.year  if sample==1, vce(cluster LGAFINAL21)
est store reg1 

xi: reg growproduc_serv_share Xit2 i.year if sample==1, vce(cluster LGAFINAL21)
est store reg2 

xi: reg growpublic_serv_share Xit2 i.year if sample==1, vce(cluster LGAFINAL21)
est store reg3 
...
// Table 1 (Panel B) : FE
est clear
xi: xtivreg2 growconsum_serv_share Xit2 i.year if sample==1, fe cluster(LGAFINAL21) savefprefix(fs_) 
est store m1

xi: xtivreg2 growproduc_serv_share Xit2 i.year if sample==1, fe cluster(LGAFINAL21) savefprefix(fs_) 
est store m2 

xi: xtivreg2 growpublic_serv_share Xit2 i.year if sample==1, fe cluster(LGAFINAL21) savefprefix(fs_) 
est store m3 
...
xi: xtivreg2 growproduc_serv_share (Xit2 = Zit2) i.year $demo  if sample==1, fe cluster(LGAFINAL21) robust first savefprefix(fs_) 
est store m2 
...
est clear 
xi: xtivreg2 growunempl_rate (Xit2 = Zit2) i.year $demo if sample==1&metro2==1, fe cluster(LGAFINAL21) robust first savefprefix(fs_) 
est store m1
...
xi: xtivreg2 growunempl_rate (Xit2 = Zit2) i.year $demo if sample==1&metro2==0, fe cluster(LGAFINAL21) robust first savefprefix(fs_) 
est store m4
...
est clear 
xi: xtivreg2 growconsum_serv_share (Xit2 = Zit2) i.year $demo if sample==1&metro2==1, fe cluster(LGAFINAL21) robust first savefprefix(fs_) 
est store m1

xi: xtivreg2 growproduc_serv_share (Xit2 = Zit2) i.year $demo if sample==1&metro2==1, fe cluster(LGAFINAL21) robust first savefprefix(fs_) 
est store m2 

xi: xtivreg2 growpublic_serv_share (Xit2 = Zit2) i.year $demo if sample==1&metro2==1, fe cluster(LGAFINAL21) robust first savefprefix(fs_) 
est store m3 

xi: xtivreg2 growconsum_serv_share (Xit2 = Zit2) i.year $demo if sample==1&metro2==0, fe cluster(LGAFINAL21) robust first savefprefix(fs_) 
est store m4

xi: xtivreg2 growproduc_serv_share (Xit2 = Zit2) i.year $demo if sample==1&metro2==0, fe cluster(LGAFINAL21) robust first savefprefix(fs_) 
est store m5 

xi: xtivreg2 growpublic_serv_share (Xit2 = Zit2) i.year $demo if sample==1&metro2==0, fe cluster(LGAFINAL21) robust first savefprefix(fs_) 
est store m6 
