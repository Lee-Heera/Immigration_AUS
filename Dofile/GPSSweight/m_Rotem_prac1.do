...
**********************************
qui xi: reg manu_share i.year i.LGAFINAL21 , cl(LGAFINAL21)
predict yhat, xb
gen p_t_res=manu_share-yhat
...
scatter Beta KPF [w=abs_alpha] if alpha_hat >= 0, msymbol(Oh) || scatter Beta KPF [w=abs_alpha] if alpha_hat < 0, msymbol(Dh) legend(order(1 "Positive weights" 2 "Negative weights") pos(5) ring(0) size(small) region(fcolor(none) lcolor(black) lwidth(thin))) yline(-0.035) xlabel(0(10)50, nogrid) ylabel(-0.8(0.1)0.1, nogrid)
