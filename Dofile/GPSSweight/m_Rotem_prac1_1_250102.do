...
drop if grow_unempl_rate==.

tabulate year, generate(year_)
********

* residualize 
xi: reg grow_unempl_rate D1_share_popfifold D1_share_college year_2-year_5 i.LGAFINAL21, cl(LGAFINAL21)
predict yhat, xb
gen p_t_res=grow_unempl_rate-yhat

xi: reg Xit2 D1_share_popfifold D1_share_college year_2-year_5 i.LGAFINAL21, cl(LGAFINAL21) //샘플수 2480개라서 2474개로 맞추기 
predict xhat, xb
...
xtivreg28 grow_unempl_rate (Xit2 = z_iv) D1_share_popfifold D1_share_college year_2-year_5, fe cl(LGAFINAL21) first savefirst savefprefix(st`i') 

/*
ivreg2 grow_unempl_rate (Xit2 = z_iv) D1_share_popfifold D1_share_college year_2-year_5, cl(LGAFINAL21) first savefirst savefprefix(st`i') 

ivreg2 p_t_res (immigration_res = shift912), cl(LGAFINAL21) first savefirst savefprefix(st`i') 

ivreg2 p_t_res (immigration_res = shift913), cl(LGAFINAL21) first savefirst savefprefix(st`i') nocons
...
esttab reg* using rotem_ivreg_manu.csv, nogap stats(N cdf arf arfp widstat) title("Table: IV regression with each share") r2(%8.3f) b(%8.3f) se(%8.3f) star(* 0.10 ** 0.05 *** 0.01) replace
