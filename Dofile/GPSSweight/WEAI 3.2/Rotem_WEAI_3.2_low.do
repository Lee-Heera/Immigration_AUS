...
* residualize y
xi: reg share_lowsk $demo year_1-year_5 i.LGAFINAL21 if sample==1 , cl(LGAFINAL21)
predict yhat, xb
gen p_t_res = share_lowsk - yhat
...
scalar b_overall_lowsk = _b[immigration_res]
display "Rotemberg base coef  = " %10.6f b_overall_lowsk

* MAIN results 의 coefficient 와 일치하는지 확인

cd "/Users/ihuila/Desktop/data/2025ABS/tables/rotem/3-2/lowskill"
...
display "========================================"
display "Overall IV coef      = " %10.6f b_overall_lowsk
display "Rotemberg recomposed = " %10.6f b_rotem[1]
display "Difference           = " %10.8f (b_overall_lowsk - b_rotem[1])
display "========================================"
drop ab b_rotem
...
save "Rotemberg_lowsk_results.dta", replace
***********************************************************
...
use "Rotemberg_lowsk_results.dta", clear
...
***********************************************************
preserve
//drop if KPF < 5 | KPF > 400
drop if country == "ZZZ"

display "=== b_overall_lowsk check === " %10.6f scalar(b_overall_lowsk)
...
twoway ///
    ...
    , ///
    yline(`=scalar(b_overall_lowsk)', lpattern(dash) lcolor(black) lwidth(medium)) ///
...
file write tex "\caption{Summary of Rotemberg Weights -- Low skill}" _n
file write tex "\label{TA:rotem_lowsk}" _n
