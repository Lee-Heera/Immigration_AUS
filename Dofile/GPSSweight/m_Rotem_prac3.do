...
**********************************
qui xi: reg serv_share i.year i.LGAFINAL21 , cl(LGAFINAL21)
predict yhat, xb
gen p_t_res=serv_share-yhat

qui xi: reg g_kt i.year i.LGAFINAL21 , cl(LGAFINAL21)
predict xhat, xb
gen immigration_res=g_kt-xhat

save "Immi_res.dta", replace
***********************************************************
forv i = 1/28{
	gen rw_denom`i' = g_kt_`i' * share_cob`i'_91 * immigration_res
}
...
mkmat g_kt_1 - g_kt_28 in 1/1, matrix(Gr)
...
scatter Beta KPF [w=abs_alpha] if alpha_hat >= 0, msymbol(Oh) || scatter Beta KPF [w=abs_alpha] if alpha_hat < 0, msymbol(Dh) legend(order(1 "Positive weights" 2 "Negative weights") pos(5) ring(0) size(small) region(fcolor(none) lcolor(black) lwidth(thin))) yline(-0.3056) xlabel(0(10)50, nogrid) ylabel(-1(0.2)1, nogrid)
