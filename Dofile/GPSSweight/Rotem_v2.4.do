...
* empl_rate
gen empl_rate1 = employed / popfifteen
...
drop if growempl_rate1 ==. // obs = 2,480 
...
* residualize 
xi: reg growempl_rate1 D1_share_popfifold D1_share_college year_2-year_5 i.LGAFINAL21, cl(LGAFINAL21)
predict yhat, xb
gen p_t_res=growempl_rate1-yhat
...
xtivreg28 growempl_rate1 (Xit2 = z_iv) D1_share_popfifold D1_share_college year_2-year_5, fe cl(LGAFINAL21) first savefirst savefprefix(st`i') 

cd "/Users/ihuila/Desktop/data/2025ABS/tables/rotem"
...
esttab reg* using rotem_ivreg_empl.csv, nogap stats(N cdf arf arfp widstat) title("Table: IV regression with each share") r2(%8.3f) b(%8.3f) se(%8.3f) star(* 0.10 ** 0.05 *** 0.01) replace
esttab st* using rotem_ivreg_empl.csv, nogap stats(N cdf arf arfp) title("Table: IV regression with each share") r2(%8.3f) b(%8.3f) se(%8.3f) star(* 0.10 ** 0.05 *** 0.01) append 
...
scatter Beta KPF [w=abs_alpha] if alpha_hat >= 0, msymbol(Oh) || scatter Beta KPF [w=abs_alpha] if alpha_hat < 0, msymbol(Dh) legend(order(1 "Positive weights" 2 "Negative weights") pos(5) ring(0) size(small) region(fcolor(none) lcolor(black) lwidth(thin))) yline(-0.62309) xlabel(0(10)50, nogrid) ylabel(-1(0)1, nogrid)
