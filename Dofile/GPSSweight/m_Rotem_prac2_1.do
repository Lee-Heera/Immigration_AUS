...
qui xi: reg immi_kt_s i.year i.LGAFINAL21 , cl(LGAFINAL21)
predict xhat, xb
gen immigration_res=immi_kt_s-xhat

save "Immi_res.dta", replace
***********************************************************
forv i = 1/28{
	gen rw_denom`i' = immi_kt_`i'_s * share_cob`i'_91 * immigration_res
}
...
mkmat immi_kt_1_s - immi_kt_28_s in 1/1, matrix(Gr)
...
scatter Beta KPF [w=abs_alpha] if alpha_hat >= 0, msymbol(Oh) || scatter Beta KPF [w=abs_alpha] if alpha_hat < 0, msymbol(Dh) legend(order(1 "Positive weights" 2 "Negative weights") pos(5) ring(0) size(small) region(fcolor(none) lcolor(black) lwidth(thin))) yline(-0.004) xlabel(0(10)50, nogrid) ylabel(-0.8(0.1)0.1, nogrid)
